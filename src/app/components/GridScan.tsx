...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (uLineStyle > 0.5) {
      ...
      lineX2 *= step(vy2m, dashDuty2);
      lineY2 *= step(vx2m, dashDuty2);
    }
...
export function GridScan({
  ...
  useEffect(() => {
    ...
    const onWindowMouseOut = (e: Event) => {
      const me = e as MouseEvent;
      if (!me.relatedTarget && (me.clientX <= 0 || me.clientY <= 0 || me.clientX >= window.innerWidth || me.clientY >= window.innerHeight)) {
        onLeave();
      }
    };
    const target = trackWindow ? window : el;
    target.addEventListener('mousemove', onMove as EventListener);
    el.addEventListener('mouseenter', onEnter);
    el.addEventListener('mouseleave', onLeave);
    if (trackWindow) document.addEventListener('mouseout', onWindowMouseOut);
    return () => {
      target.removeEventListener('mousemove', onMove as EventListener);
      el.removeEventListener('mouseenter', onEnter);
      el.removeEventListener('mouseleave', onLeave);
      if (trackWindow) document.removeEventListener('mouseout', onWindowMouseOut);
      if (leaveTimer) clearTimeout(leaveTimer);
    };
  }, [snapBackDelay, trackWindow]);
  ...
  useEffect(() => {
    ...
    if (enablePost) {
      ...
      (bloom as any).blendMode.opacity.value = Math.max(0, bloomIntensity);
  ...
  return (
    <div
      ref={containerRef}
      className={className ? `gridscan ${className}` : 'gridscan'}
      style={{
        position: 'absolute',
        inset: 0,
        width: '100%',
